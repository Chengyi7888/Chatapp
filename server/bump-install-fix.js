...
for (const m of [ROOT + '/android/AndroidManifest.xml', ROOT + '/build/AndroidManifest.xml']) {
  patch(m, [
    ...
    ['android:versionName="2.1.1.2.1.2.2"', 'android:versionName="2.1.1.2.1.2.2.1"'],
  ]);
}
patch(ROOT + '/android/src/com/chatapp/app/MainActivity.java', [
  ['"App v2.1.1.2.1.2.2"', '"App v2.1.1.2.1.2.2.1"'],
]);

const home = ROOT + '/android/src/com/chatapp/app/HomeActivity.java';
let h = fs.readFileSync(home, 'utf8');
const pairs = [
  ['"关于 (App v2.1.1.2.1.2.2)"', '"关于 (App v2.1.1.2.1.2.2.1)"'],
  ['"Chatapp v2.1.1.2.1.2.2\\n" + I18n.t("current_server")', '"Chatapp v2.1.1.2.1.2.2.1\\n" + I18n.t("current_server")'],
  ['        addVersion(list, "v2.1.1.2.1.2.2", new String[]{"群聊右上角三点直接进入群聊信息", "移除群聊信息页的\\u201C进入群聊\\u201D按钮", "聊天气泡新增头像(群聊含发送者姓名)"});',
   '        addVersion(list, "v2.1.1.2.1.2.2.1", new String[]{"修复更新下载后无法跳转安装(引导开启安装权限并自动重试)"});\n        addVersion(list, "v2.1.1.2.1.2.2", new String[]{"群聊右上角三点直接进入群聊信息", "移除群聊信息页的\\u201C进入群聊\\u201D按钮", "聊天气泡新增头像(群聊含发送者姓名)"});'],
];
let ok = 0, miss = [];
for (const [old, neu] of pairs) {
...
patch(ROOT + '/server/server.js', [["version: '2.1.1.2.1.2.2'", "version: '2.1.1.2.1.2.2.1'"]]);
patch(ROOT + '/outputs/server.js', [["version: '2.1.1.2.1.2.2'", "version: '2.1.1.2.1.2.2.1'"]]);
console.log('DONE');
